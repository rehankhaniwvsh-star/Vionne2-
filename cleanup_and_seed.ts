...
const cleanupAndSeed = async () => {
  console.log('Cleaning up existing data in project:', firebaseConfig.projectId);
  
  try {
    await deleteCollection('products');
    await deleteCollection('orders');
    // Also clear settings if needed, but usually we want to keep them or update them
